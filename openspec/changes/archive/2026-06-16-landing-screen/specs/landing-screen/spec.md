...
Pressing ENTER on the landing screen SHALL transition into the main game scene.

#### Scenario: ENTER loads the main game
- **WHEN** the landing screen is visible and the player presses ENTER
- **THEN** the screen transitions to the main game scene and gameplay begins
...
### Requirement: Variant selection via launch flag during review
During review the implementation SHALL offer three title style variants (differing in font, shadow offset/weight, and layout), selected by a command-line launch flag rather than any in-game control. After review the chosen variant SHALL become the single unconditional default and the flag and unused variants SHALL be removed.

#### Scenario: Launch flag selects a variant
- **WHEN** the game is launched with the variant flag set to a, b, or c
- **THEN** the landing screen displays that variant's title treatment

#### Scenario: No in-game variant selector
- **WHEN** the landing screen is displayed during normal play
- **THEN** there is no on-screen control for switching title variants

#### Scenario: Chosen variant is the kept implementation
- **WHEN** the user selects one variant after review
- **THEN** the landing screen uses that variant unconditionally and the launch flag and the other two variants are not part of the shipped screen
